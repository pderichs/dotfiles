...
(map! :leader :desc "Select all chars of current line" "lv" (edmacro-parse-keys "^v$h" t))
(map! :leader :desc "Yank all chars of current line" "ly" (edmacro-parse-keys "^v$hy" t))
...
(map! :leader :desc "Open the todo list" "to" #'org-todo-list) ;; also: SPC o a t
...
(define-key evil-normal-state-map (kbd "g s s") #'evil-avy-goto-char-2)
